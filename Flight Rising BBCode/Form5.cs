...
namespace Flight_Rising_BBCode
{
	public partial class Form5 : Form
	{
		...
		public Form5()
		{
			...
			if (Environment.UserName != "Katy")
				btnCopyHyperText.Visible = false;
		...
		private void showFonts()
		{
...

			{
				...
				listFonts.Items.Add(font[i].Name);
		...
		private void btnGenerate_Click(object sender, EventArgs e)
		{
			if (fatherID.Text != "")
			{
				GenerateImagesFather();
			}
			if (txtMotherID.Text != "")
			{
				GenerateImagesMother();
			}

			if (txtFatherName.Text != "")
				setFathersName(txtFatherName.Text);
			if (txtMotherName.Text != "")
				setMothersName(txtMotherName.Text);
		...
		{
			//get the breed info of the parents.  and calculate the probability of each parent.

			string hatchlings = "";


			if (chkRarity.Checked)
			{
				lblFatherRare.Visible = true;
				lblMotherRare.Visible = true;

				lblFatherRare.Text = ShowRare(sire).ToString();
				lblMotherRare.Text = ShowRare(carrier).ToString();

				hatchlings = "<head><style>.wrapper {position: relative; margin: 0 auto; text-align:center; width: 700px;}" + Environment.NewLine +
					".center-div {position: relative; margin: 0 auto; border-radius: 3px; position: relative; text-align:center; display:inline; width:" + paletteWidthNum.Value + "px; height:" + paletteHeightNum.Value + "px; background-color:#888;}</style>" + Environment.NewLine +
					"</head><body style=\"background-color:#f0f0f0;\"><center><div class=\"wrapper\">" + breedPercentage(sire.breed, carrier.breed) + "<br>" +
					...
					GetColors(sire.tertColor, carrier.tertColor) + "</div></center></body>";
			}
			else
			{
				lblFatherRare.Visible = false;
				lblMotherRare.Visible = false;
				hatchlings = "<head><style>.wrapper {position: relative; margin: 0 auto; text-align:center; width: 700px;}" + Environment.NewLine +
					".center-div {position: relative; margin: 0 auto; border-radius: 3px; position: relative; text-align:center; display:inline; width:" + paletteWidthNum.Value + "px; height:" + paletteHeightNum.Value + "px; background-color:#888;}</style>" + Environment.NewLine +
					"</head><body style=\"background-color:#f0f0f0;\"><center><div class=\"wrapper\">" + breedPercentage(sire.breed, carrier.breed) + "<br>" +
					primePercentageSet(sire.primeGene, carrier.primeGene, sire.breed) + "<br>" +
		...
		private object ShowRare(Dragon dragon)
		{
			return FindRarity(dragon.breed, dragon.CreateBreedList()) + Environment.NewLine +
				FindRarity(dragon.primeGene, dragon.CreatePrimaryGeneList(dragon.breed)) + Environment.NewLine +
				FindRarity(dragon.SecondGene, dragon.CreateSecondaryGeneList(dragon.breed)) + Environment.NewLine +
				FindRarity(dragon.tertGene, dragon.CreateTertGeneList(dragon.breed));
		}
		...
		private string TertPercentageSet(string breed1, string breed2, string sireBreed)
		{
			...
			string breedTypeDad = FindRarity(breed1, CompleteBreedsArray).ToLower();
			string breedTypeMom = FindRarity(breed2, CompleteBreedsArray).ToLower();
		...
		private string GenerateResult(Dragon carrier)
		{
		...
		private void setMothersName(string text)
		{
			lblMotherName.Text = text;
		}

		private void setFathersName(string text)
		{
			lblFathersName.Text = text;
		}

		private void GenerateImagesMother()
		{
			string id = txtMotherID.Text.Trim(' ');

			string batchNum = dreg.FindBatchNum(id).ToString();

			imgMother.ImageLocation = dreg.ImgURL("350", batchNum, id, "_350.png");
		}

		private void GenerateImagesFather()
		{
			string id = fatherID.Text.Trim(' ');

			string batchNum = dreg.FindBatchNum(id).ToString();

			imgFather.ImageLocation = dreg.ImgURL("350", batchNum, id, "_350.png");
		}

		private void listFonts_SelectedIndexChanged(object sender, EventArgs e)
		{
			lblFathersName.Font = new Font(listFonts.SelectedItem.ToString(), 21);

			lblMotherName.Font = new Font(listFonts.SelectedItem.ToString(), 21);
		}
		...
		private void breedFather_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			string[,] PrimeGeneList = dreg.CreatePrimaryGeneList(breedFather.SelectedItem.ToString());
			string[,] SecondGeneList = dreg.CreateSecondaryGeneList(breedFather.SelectedItem.ToString());
			string[,] tertGeneList = dreg.CreateTertGeneList(breedFather.SelectedItem.ToString());

			for (int i = 0; i < PrimeGeneList.Length / 2; i++)
			{
				primaryGeneFather.Items.Add(PrimeGeneList[i, 0]);
			}
			for (int i = 0; i < SecondGeneList.Length /2; i++)
			{
				secondaryGeneFather.Items.Add(SecondGeneList[i, 0]);
			}
			for (int i = 0; i < tertGeneList.Length /2; i++)
			{
				tertGeneFather.Items.Add(tertGeneList[i, 0]);
			}
		...
		private void breedMother_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			string[,] PrimeGeneList = dreg.CreatePrimaryGeneList(breedMother.SelectedItem.ToString());
			string[,] SecondGeneList = dreg.CreateSecondaryGeneList(breedMother.SelectedItem.ToString());
			string[,] tertGeneList = dreg.CreateTertGeneList(breedMother.SelectedItem.ToString());

			for (int i = 0; i < PrimeGeneList.Length / 2; i++)
			{
				primaryGeneMother.Items.Add(PrimeGeneList[i, 0]);
			}
			for (int i = 0; i < SecondGeneList.Length / 2; i++)
			{
				secondaryGeneMother.Items.Add(SecondGeneList[i, 0]);
			}
			for (int i = 0; i < tertGeneList.Length / 2; i++)
			{
				tertGeneMother.Items.Add(tertGeneList[i, 0]);
			}
		}

		private void btnCopyHyperText_Click(object sender, EventArgs e)
		{
			Clipboard.SetText(lblOffspringPrediction.DocumentText);
		}
	}
}
